...
h.test("is_text distinguishes text, binary and missing", function()
	h.eq(preview.is_text(tmpfile({ "hello" })), true)
	h.eq(preview.is_text(tmpfile("a\0b", "b")), false)
	h.eq(preview.is_text(vim.fn.tempname() .. "/nope"), nil)
end)

h.test("read bounds lines and reports truncation", function()
	local path = tmpfile({ "1", "2", "3", "4" })
	local lines, err, truncated = preview.read(path, 2)
	h.eq(err, nil)
	h.eq(lines, { "1", "2" })
	h.eq(truncated, true)

	local all, aerr, atrunc = preview.read(path)
	h.eq(aerr, nil)
	h.eq(#all, 4)
	h.eq(atrunc, false)
end)
...
h.test("read reports unreadable files", function()
	local lines, err = preview.read(vim.fn.tempname() .. "/nope", 10)
	h.eq(lines, nil)
	h.ok(err ~= nil)
end)
...
h.test("should_highlight is true for a small buffer", function()
	...
	h.eq(preview.should_highlight(b), true)
end)
